  ...
  loader.load(
    ...
    (xhr) => {
      if (xhr.total > 0) {
        ...
        updateStatus(`GLBモデルを読み込んでいます。${percent}%`);
      }
    },
    (error) => {
      console.error("GLB load error:", error);
      updateStatus("GLBモデルの読み込みに失敗しました。models/forklift_ky.glb の場所とファイル名を確認してください。");
    }
  );
}
...
function onSelect() {
  if (!reticle.visible || !loadedModel) {
    updateStatus("床面が見つかっていません。スマートフォンをゆっくり左右に動かしてください。");
    return;
  }
  ...
  if (!modelPlaced) {
    ...
    modelPlaced = true;
    updateStatus("モデルを配置しました。危険エリア、作業者の位置、フォークリフトの後退動線を確認してください。再生ボタンで動きを確認できます。");
  } else if (placedModel) {
    placedModel.position.setFromMatrixPosition(reticle.matrix);
    placedModel.quaternion.setFromRotationMatrix(reticle.matrix);
  ...
  if (frame) {
    ...
    if (hitTestSource) {
      ...
      if (hitTestResults.length) {
        ...
        if (!modelPlaced) updateStatus("床面を検出しました。画面をタップしてモデルを配置してください。");
      } else {
        reticle.visible = false;
        if (!modelPlaced) updateStatus("床面を探しています。スマートフォンをゆっくり動かしてください。");
      }
    }
  }
